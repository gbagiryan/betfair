  ...
  try {
    ...
    for (const eType of eventTypes) {
      ...
      for (const com of Object.keys(eType.competitions)) {
        eType.competitions[com].events = await apiCalls.requestEventListForCompetition(eType.competitions[com].id);
        for (const event of Object.keys(eType.competitions[com].events)) {
          ...
          for (const market of Object.keys(eType.competitions[com].events[event].markets)) {
            eType.competitions[com].events[event].markets[market].marketBooks = await apiCalls.requestMarketBookForMarket(eType.competitions[com].events[event].markets[market].marketId);
          }
        }
      }
    }
